...
test.describe('Texas Electricity Calculator', () => {
  ...
  test('should calculate plans for a medium house', async ({ page }) => {
    ...
    await expect(page.locator('#step-usage')).not.toHaveClass(/calc-step-disabled/, { timeout: 10000 });
  ...
  test('should display proper warning badges for TIME OF USE plans', async ({ page }) => {
    ...
    if (await touBadge.isVisible()) {
      ...
      // Verify SVG warning icon is present (using ::before is hard to test directly, but we can check if it has the class/pseudo-element effect if we had more complex tests)
      // For now, existence is a good sign.
    }
  });
