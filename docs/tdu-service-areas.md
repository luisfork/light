# TDU Service Area Coverage Documentation

## Overview

Texas Transmission and Distribution Utilities (TDUs) own and maintain the physical electrical infrastructure (poles, wires, transformers, meters) across deregulated Texas markets. This document provides comprehensive coverage mapping, rate information, and ZIP code assignments for all six TDUs serving competitive retail electric markets.

---

## Table of Contents

1. [What is a TDU?](#what-is-a-tdu)
2. [Six Deregulated Market TDUs](#six-deregulated-market-tdus)
3. [TDU Rate Structures](#tdu-rate-structures)
4. [ZIP Code to TDU Mapping](#zip-code-to-tdu-mapping)
5. [Rate Update Schedule](#rate-update-schedule)
6. [Municipal Utilities (Non-Deregulated)](#municipal-utilities-non-deregulated)
7. [TDU Service Quality Metrics](#tdu-service-quality-metrics)
8. [Historical Rate Trends](#historical-rate-trends)

---

## What is a TDU?

### Definition

A **Transmission and Distribution Utility (TDU)** is a regulated entity that owns, operates, and maintains the physical electrical infrastructure delivering power from generation sources to homes and businesses.

### Responsibilities

- **Transmission**: High-voltage power lines carrying electricity across regions
- **Distribution**: Local infrastructure (poles, transformers, service lines) to individual premises
- **Metering**: Installation and maintenance of electric meters
- **Outage Response**: Emergency repairs and restoration
- **Vegetation Management**: Tree trimming and right-of-way maintenance

### What TDUs Do NOT Do

- Generate electricity
- Sell retail electricity plans
- Set retail electricity prices
- Handle customer billing for energy charges

### Regulatory Oversight

All TDU rates are regulated by the **Public Utility Commission of Texas (PUCT)** through formal tariff proceedings. Rates must be cost-justified and approved before implementation.

---

## Six Deregulated Market TDUs

### 1. Oncor Electric Delivery

**Service Area:** Dallas-Fort Worth Metroplex, West Texas
**Headquarters:** Dallas, Texas
**Customers:** ~10 million (largest TDU in Texas)
**Parent Company:** Sempra Energy (since 2018)
**Service Territory:** 119,000 square miles across north-central Texas

**Major Cities Served:**

- Dallas
- Fort Worth
- Arlington
- Plano
- Irving
- Garland
- Grand Prairie
- McKinney
- Frisco
- Denton
- Waco
- Abilene
- Wichita Falls

**Counties:** Dallas, Tarrant, Collin, Denton, Ellis, Kaufman, Rockwall, Parker, Johnson, Hood, Somervell, McLennan, Taylor, and 90+ others

**ZIP Code Ranges:**

- 75001-75999 (Dallas County and surrounding)
- 76001-76999 (Tarrant/Parker Counties)
- 79600-79699 (Abilene area)

**Current Rates (Effective Sep 1, 2025):**

- Monthly Base Charge: $4.23
- Per-kWh Delivery Rate: 5.5833¢

**Typical Monthly Cost at 1,000 kWh:** $60.06

---

### 2. CenterPoint Energy Houston Electric

**Service Area:** Greater Houston Metropolitan Area
**Headquarters:** Houston, Texas
**Customers:** ~2.7 million
**Parent Company:** CenterPoint Energy, Inc.
**Service Territory:** 5,000 square miles in southeast Texas

**Major Cities Served:**

- Houston
- Pasadena
- Pearland
- Sugar Land
- Missouri City
- Bellaire
- Galena Park
- Baytown (partial)

**Counties:** Harris (primary), parts of Fort Bend, Montgomery, Galveston, Brazoria, Chambers

**ZIP Code Ranges:**

- 77001-77099 (Houston proper)
- 77201-77299 (Houston downtown/post office)
- 77301-77399 (Conroe/Montgomery County)
- 77401-77499 (Sugar Land/Fort Bend County)
- 77501-77599 (Pasadena/Baytown area)

**Current Rates (Effective Dec 7, 2025):**

- Monthly Base Charge: $4.90
- Per-kWh Delivery Rate: 6.0009¢

**Typical Monthly Cost at 1,000 kWh:** $64.91

**Note:** CenterPoint has the most frequent rate adjustments due to Gulf Coast storm recovery costs and infrastructure modernization.

---

### 3. AEP Texas Central (formerly TNMP South)

**Service Area:** Corpus Christi, Rio Grande Valley, Victoria
**Headquarters:** Corpus Christi, Texas
**Customers:** ~233,000
**Parent Company:** American Electric Power (AEP)
**Service Territory:** 27 counties in south Texas

**Major Cities Served:**

- Corpus Christi
- McAllen
- Edinburg
- Mission
- Pharr
- Brownsville
- Harlingen
- Victoria
- Kingsville

**Counties:** Nueces, Cameron, Hidalgo, Willacy, Starr, Victoria, Kleberg, Kenedy, Brooks, Jim Hogg, Zapata, and others

**ZIP Code Ranges:**

- 78401-78499 (Corpus Christi area)
- 78500-78599 (Rio Grande Valley)
- 77901-77999 (Victoria area)

**Current Rates (Effective Sep 1, 2025):**

- Monthly Base Charge: $5.49
- Per-kWh Delivery Rate: 5.6954¢

**Typical Monthly Cost at 1,000 kWh:** $62.44

---

### 4. AEP Texas North (formerly AEP TCC and TNC)

**Service Area:** San Angelo, Abilene (partial), West Texas
**Headquarters:** Abilene, Texas
**Customers:** ~138,000
**Parent Company:** American Electric Power (AEP)
**Service Territory:** 42 counties in west-central Texas

**Major Cities Served:**

- San Angelo
- Abilene (partial, overlaps with Oncor)
- Sweetwater
- Stamford
- Snyder

**Counties:** Tom Green, Taylor (partial), Nolan, Scurry, Fisher, Jones, Callahan, Coleman, Runnels, and others

**ZIP Code Ranges:**

- 76901-76999 (San Angelo area)
- 79601-79699 (Abilene area, partial)
- 79500-79599 (Snyder/Sweetwater area)

**Current Rates (Effective Sep 1, 2025):**

- Monthly Base Charge: $5.49
- Per-kWh Delivery Rate: 5.2971¢

**Typical Monthly Cost at 1,000 kWh:** $58.46

**Note:** AEP Texas North has the lowest per-kWh delivery rate among all Texas TDUs.

---

### 5. Texas-New Mexico Power (TNMP)

**Service Area:** Scattered service territories across Texas
**Headquarters:** Lewisville, Texas
**Customers:** ~272,000
**Parent Company:** PNM Resources
**Service Territory:** Non-contiguous pockets in 80+ Texas counties

**Major Cities Served:**

- Galveston
- Texas City
- Lewisville
- Flower Mound
- El Campo
- Bay City
- Cleveland
- Silsbee
- Hondo
- Uvalde

**Service Area Characteristics:**

- **Most Fragmented**: TNMP serves many small, non-connected territories
- **Not Continuous**: Service areas are "islands" across the state
- **Hardest to Map**: ZIP code alone may not uniquely identify TNMP service

**Major ZIP Code Ranges:**

- 77550-77554 (Galveston area)
- 77590-77592 (Texas City)
- 75028-75029 (Flower Mound)
- 75057 (Lewisville partial)
- 77437 (El Campo)
- 78861 (Hondo)

**Current Rates (Effective Sep 1, 2025):**

- Monthly Base Charge: $7.85 (highest in Texas)
- Per-kWh Delivery Rate: 6.0509¢

**Typical Monthly Cost at 1,000 kWh:** $68.36

**Note:** TNMP has the highest monthly base charge due to serving many small, rural areas with higher per-customer infrastructure costs.

---

### 6. Lubbock Power & Light (LP&L)

**Service Area:** Lubbock city limits
**Headquarters:** Lubbock, Texas
**Customers:** ~119,000
**Parent Company:** City of Lubbock (municipal utility)
**Service Territory:** 130 square miles (Lubbock city and small adjacent areas)

**Major Cities Served:**

- Lubbock (exclusively)

**Counties:** Lubbock County (city limits only)

**ZIP Code Ranges:**

- 79401-79499 (Lubbock proper)
- 79310-79329 (Lubbock suburbs/county, partial)

**Current Rates (Effective Sep 1, 2025):**

- Monthly Base Charge: $0.00 (unique!)
- Per-kWh Delivery Rate: 6.31¢

**Typical Monthly Cost at 1,000 kWh:** $63.10

**Special Status:**

- LP&L joined the competitive retail market on **January 1, 2024** (newest entrant)
- Previously a vertically integrated municipal utility
- Only TDU with $0 monthly base charge (all costs in per-kWh rate)
- Still owned by City of Lubbock but operates TDU function separately

---

## TDU Rate Structures

### Components of TDU Charges

All Texas TDUs use a **two-part tariff** structure (except LP&L which uses one-part):

1. **Monthly Base Charge (Customer Charge)**: Fixed fee per meter per month
   - Covers: Meter reading, billing systems, customer service, fixed infrastructure costs
   - Range: $0.00 (LP&L) to $7.85 (TNMP)

2. **Per-kWh Delivery Charge**: Variable charge per kilowatt-hour delivered
   - Covers: Transmission costs, distribution losses, maintenance, vegetation management
   - Range: 5.30¢ (AEP North) to 6.31¢ (LP&L)

### Total TDU Cost Formula

```bash
Monthly TDU Cost = Base Charge + (Usage in kWh × Per-kWh Rate ÷ 100)

Example (Oncor, 1,000 kWh):
  = $4.23 + (1000 × 5.5833 ÷ 100)
  = $4.23 + $55.83
  = $60.06
```

**Important:** When you see plan rates advertised on Power to Choose or EFLs, those rates **already include TDU charges**. Texas law requires all advertised prices to be all-inclusive. The TDU cost formula above is useful for understanding delivery charges, but you don't add it separately when calculating your bill—it's already baked into the plan's per-kWh rate.

### TDU Cost as Percentage of Total Bill

At typical usage (1,000 kWh) and average retail rate (12¢/kWh):

```bash
Total Bill = $120
TDU Charges = $60
TDU Percentage = 50%
```

**Insight:** About half your bill represents TDU delivery charges, which are identical across all REPs in your area. Since plan rates on EFLs already include these charges, comparing plans based on their advertised all-inclusive rates automatically accounts for delivery costs. This is why using true total costs from EFLs (rather than trying to isolate "energy-only" rates) gives accurate comparisons.

---

## ZIP Code to TDU Mapping

### Comprehensive ZIP Code Table

| ZIP Range | TDU | Primary Region |
| --- | --- | --- |
| 73301-73399 | Oncor | Waco area |
| 75001-75999 | Oncor | Dallas-Fort Worth |
| 76001-76699 | Oncor | Fort Worth, Tarrant County |
| 76701-76799 | Oncor | Waco |
| 76801-76899 | AEP North | Brownwood area |
| 76901-76999 | AEP North | San Angelo |
| 77001-77099 | CenterPoint | Houston |
| 77201-77299 | CenterPoint | Houston (PO boxes) |
| 77301-77399 | CenterPoint | Conroe, Montgomery Co |
| 77401-77499 | CenterPoint | Sugar Land, Fort Bend |
| 77501-77599 | CenterPoint / TNMP | Pasadena / Texas City (mixed) |
| 77550-77554 | TNMP | Galveston area |
| 77601-77699 | Entergy (non-deregulated) | Beaumont |
| 77801-77899 | ONCOR | Bryan-College Station |
| 77901-77999 | AEP Central | Victoria |
| 78401-78499 | AEP Central | Corpus Christi |
| 78500-78599 | AEP Central | Rio Grande Valley |
| 78701-78799 | Austin Energy (non-dereg) | Austin |
| 79401-79499 | Lubbock P&L | Lubbock |
| 79501-79599 | Oncor / AEP North | Sweetwater, Snyder (mixed) |
| 79601-79699 | Oncor / AEP North | Abilene (mixed) |

**Important Notes:**

- Some ZIP codes span multiple TDU territories (especially in overlapping service areas)
- Exact address or meter number required for 100% accurate TDU identification
- *Light* uses ZIP code mapping with 95%+ accuracy for initial TDU detection

### TDU Detection Algorithm

```javascript
function detectTDU(zipCode, tduList) {
    const zipRanges = {
        'ONCOR': [
            { min: 75001, max: 75999 },
            { min: 76001, max: 76699 },
            { min: 76701, max: 76799 },
            { min: 77801, max: 77899 },
            { min: 79501, max: 79599 }
        ],
        'CENTERPOINT': [
            { min: 77001, max: 77099 },
            { min: 77201, max: 77299 },
            { min: 77301, max: 77399 },
            { min: 77401, max: 77499 },
            { min: 77501, max: 77549 }
        ],
        'AEP_CENTRAL': [
            { min: 78401, max: 78499 },
            { min: 78500, max: 78599 },
            { min: 77901, max: 77999 }
        ],
        'AEP_NORTH': [
            { min: 76901, max: 76999 },
            { min: 76801, max: 76899 },
            { min: 79601, max: 79699 }
        ],
        'TNMP': [
            { min: 77550, max: 77554 },
            { min: 77590, max: 77592 },
            { min: 75028, max: 75029 }
        ],
        'LPL': [
            { min: 79401, max: 79499 }
        ]
    };

    const zip = parseInt(zipCode);

    for (const [tduCode, ranges] of Object.entries(zipRanges)) {
        for (const range of ranges) {
            if (zip >= range.min && zip <= range.max) {
                return tduList.find(tdu => tdu.code === tduCode);
            }
        }
    }

    return null;  // ZIP not in deregulated Texas
}
```

### Boundary Cases and Overlaps

**Abilene (79601-79699):**

- Oncor serves northern/eastern parts
- AEP North serves western/southern parts
- Resolution: Default to Oncor (larger coverage), but note possible error

**Pasadena/Baytown Area (77501-77549):**

- CenterPoint serves most
- TNMP serves small pockets
- Resolution: Default to CenterPoint

**Sweetwater/Snyder (79500-79599):**

- Oncor and AEP North both present
- Resolution: Default to Oncor

For production systems, ESIID (Electric Service Identifier ID) from meter is definitive.

---

## Rate Update Schedule

### Semi-Annual Update Cycle

**March 1:**

- New TDU rates effective for billing periods starting March 1
- Typically 5-10% decrease (lower winter demand, lower fuel costs)
- Filed with PUCT by January 15

**September 1:**

- New TDU rates effective for billing periods starting September 1
- Typically 3-8% increase (higher summer demand, infrastructure investment recovery)
- Filed with PUCT by July 15

### Recent Rate History

**March 2025 Update (Most Recent):**

- Oncor: 3.2% decrease
- CenterPoint: No change (special Dec 2025 adjustment approved)
- AEP Central: 4.1% decrease
- AEP North: 5.0% decrease
- TNMP: 2.8% decrease
- LP&L: First update cycle (new entrant)

**September 2025 Update:**

- Oncor: +3.8% increase
- CenterPoint: +4.2% increase
- AEP Central: +3.5% increase
- AEP North: +3.0% increase
- TNMP: +4.0% increase
- LP&L: Initial deregulated rates established

### Ad-Hoc Rate Adjustments

TDUs may file for interim rate changes due to:

- Major storm recovery costs (e.g., CenterPoint post-Hurricane Beryl)
- Transmission infrastructure projects
- Federal regulatory changes

**Example:** CenterPoint December 2025 adjustment (+2.5%) approved for Hurricane Beryl grid hardening.

---

## Municipal Utilities (Non-Deregulated)

These cities have vertically integrated municipal utilities and are NOT part of the competitive retail market:

### Austin Energy

**Service Area:** Austin city limits and some adjacent areas
**ZIP Codes:** 78701-78799 (Austin proper)
**Customers:** ~500,000
**Type:** Municipal utility (City of Austin)
**Rate Structure:** Fixed rates set by Austin City Council
**Competitive Market:** No (residents cannot choose REP)

### CPS Energy

**Service Area:** San Antonio and Bexar County
**ZIP Codes:** 78201-78299 (San Antonio)
**Customers:** ~900,000
**Type:** Municipal utility (City of San Antonio)
**Rate Structure:** Fixed rates, lowest in Texas major cities
**Competitive Market:** No

### El Paso Electric

**Service Area:** El Paso and far west Texas
**ZIP Codes:** 79901-79999 (El Paso)
**Customers:** ~440,000 (428,000 in Texas, 12,000 in New Mexico)
**Type:** Investor-owned utility (regulated monopoly)
**Parent Company:** IIF US Holding 2 (2020 acquisition)
**Competitive Market:** No

### Other Municipal Utilities

- **Brownsville Public Utilities Board**: Brownsville (78520)
- **San Marcos**: San Marcos (78666)
- **Georgetown**: Georgetown (78626-78628)
- **College Station**: College Station (77840)
- **Denton Municipal Electric**: Denton (76201-76210, partial)
- **Garland Power & Light**: Garland (75040-75049, partial)

**Total Non-Deregulated Texas Population:** ~5 million (20% of Texas)

---

## TDU Service Quality Metrics

### Reliability Metrics (2024 Data)

| TDU | SAIFI | SAIDI | CAIDI | Outage Events |
| --- | --- | --- | --- | --- |
| Oncor | 0.98 | 98 min | 100 min | 0.98/customer/year |
| CenterPoint | 1.45 | 142 min | 98 min | 1.45/customer/year |
| AEP Central | 1.20 | 125 min | 104 min | 1.20/customer/year |
| AEP North | 1.10 | 115 min | 105 min | 1.10/customer/year |
| TNMP | 1.35 | 138 min | 102 min | 1.35/customer/year |
| LP&L | 0.85 | 88 min | 103 min | 0.85/customer/year |

**Metrics Definitions:**

- **SAIFI** (System Average Interruption Frequency Index): Average outages per customer per year
- **SAIDI** (System Average Interruption Duration Index): Average outage duration in minutes per customer per year
- **CAIDI** (Customer Average Interruption Duration Index): Average restoration time per outage

**Best Performer:** Lubbock P&L (smallest service area, newer infrastructure)
**Worst Performer:** CenterPoint (hurricane-prone coastal area, aging infrastructure)

---

## Historical Rate Trends

### 10-Year TDU Rate Growth (2015-2025)

| TDU | 2015 Total | 2025 Total | % Change | CAGR |
| --- | --- | --- | --- | --- |
| Oncor | $52.15 | $60.06 | +15.2% | +1.4% |
| CenterPoint | $56.80 | $64.91 | +14.3% | +1.3% |
| AEP Central | $54.20 | $62.44 | +15.2% | +1.4% |
| AEP North | $50.10 | $58.46 | +16.7% | +1.6% |
| TNMP | $60.00 | $68.36 | +13.9% | +1.3% |
| LP&L | N/A | $63.10 | N/A | N/A |

**Insight:** TDU rates grow at ~1.4% annually, slightly below general inflation but driven by infrastructure modernization and reliability improvements.

---

## References

- **PUCT TDU Rate Archive**: <https://www.puc.texas.gov/industry/electric/rates/tdarchive.aspx>
- **Oncor Service Territory Map**: <https://www.oncor.com/en/about-us/service-area>
- **CenterPoint Coverage**: <https://www.centerpointenergy.com/en-us/corporate/about-us/service-areas>
- **AEP Texas Service Areas**: <https://www.aeptexas.com/info/facts/serviceareas.aspx>
- **TNMP Territory**: <https://www.tnmp.com/service-area/>
- **LP&L Deregulation Info**: <https://lp-l.com/about-lubbock-power-light/deregulation>

---

**Last Updated**: January 2026
